...
def load_dataset(dataset_str, data_root, batch_size, test_batch_size, num_workers, test_mode='medium', img_fmt='png'):
    ...
    elif dataset_str == 'vimeo90k3':
        from data.vimeo90k3 import get_loader
    #
    elif dataset_str == 'vimeo90Kd':
        from data.vimeo90Kd import get_loader
...
def save_checkpoint(state, is_best, exp_name, filename='checkpoint.pth'):
    ...
    try:
        shutil.copyfile(filename, f'checkpoint/{state}/model.pth')
    except:
        print("wtf i did?")
    if is_best:
...
def save_image(img, path):
    ...
    if len(img.size()) == 2:    # grayscale image
        ...
        im = Image.fromarray(q_im.permute(1, 2, 0).cpu().numpy().astype(np.uint8), 'RGBA')
    else:
        pass
    im = np.array(im)
